...
if ($PrepareRetained) {
    ...
    try {
        $env:APP_ENV = 'test'; $env:ARCHITEX_DATA_MODE = 'prototype'; $env:ARCHITEX_ENABLE_DEMO_SEED = '1'; $env:DB_HOST = $DbHost; $env:DB_NAME = $SchemaName; $env:DB_USER = $DbUser; $env:DB_PASS = $DbPass; $env:JWT_SECRET = [Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes(([Guid]::NewGuid().ToString('N') + [Guid]::NewGuid().ToString('N'))))
