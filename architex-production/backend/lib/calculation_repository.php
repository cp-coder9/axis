...
final class MariaDbCalculationRepository
{
    ...
    public function list(array $identity, ?string $projectId, bool $allForPlatform): array
    {
        ...
        elseif (!$allForPlatform) { $sql .= ' AND project_id IS NULL AND author_user_id = :actor'; $params[':actor'] = $identity['sub']; }
    ...
    private function canAccess(array $identity, array $row): bool { return $row['project_id'] === null ? ($row['author_user_id'] === $identity['sub'] || ($identity['role'] ?? '') === 'platform_admin') : (in_array('*', $identity['projects'] ?? [], true) || in_array($row['project_id'], $identity['projects'] ?? [], true)); }
