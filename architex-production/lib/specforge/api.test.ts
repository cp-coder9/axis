...
const localIdentityHeaders = vi.hoisted(() => vi.fn(() => ({ 'X-Architex-Role': 'architect' })));

vi.mock('@/lib/auth-session', () => ({ authenticatedFetch }));
vi.mock('@/lib/api', () => ({ API_BASE_URL: '/api/v1', localIdentityHeaders }));

import { SpecForgeApiError, specForgeApi } from '@/lib/specforge/api';

const identity = { role: 'architect', userId: 'user-1' } as const;
const response = (body: unknown, status = 200) => new Response(JSON.stringify(body), { status, headers: { 'Content-Type': 'application/json' } });

describe('SpecForge API client', () => {
  beforeEach(() => {
    authenticatedFetch.mockReset();
    localIdentityHeaders.mockClear();
  });

  it('loads and maps a persisted aggregate without constructing authorization headers', async () => {
    ...
    const workspace = await specForgeApi.get('project-1', identity);
    ...
    expect(new Headers(init.headers).get('X-Architex-Role')).toBe('architect');
  });
  ...
  it('adds concurrency and idempotency headers to mutations', async () => {
    ...
    await specForgeApi.createItem('project-1', { title: 'Tile' } as never, identity, 'create-key');
    await specForgeApi.updateItem('project-1', 'item-1', { title: 'Tile revised' }, 4, identity);
