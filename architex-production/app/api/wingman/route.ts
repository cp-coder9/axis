...
import { runWingmanCompletion, isValidProviderConfig } from '@/lib/wingman-providers';
import { WingmanProvider } from '@/lib/wingman-providers';

/**
 * Architex Wingman — multi-provider AI copilot endpoint.
 *
 * Accepts a bring-your-own-API (BYOAPI) provider config from the client so
 * users can use their own OpenAI / NVIDIA NIM / Gemini keys.
 */

export async function POST(req: NextRequest) {
  try {
    const body = await req.json();
    const { prompt, capability, projectContext, roleContext, documentContext, summaryMode } = body;

    /* ── Resolve the active provider ── */
    let provider: WingmanProvider = 'gemini';
    let apiKey = process.env.GEMINI_API_KEY || '';
    let model = 'gemini-2.5-flash';

    const userConfig = body.providerConfig;
    if (isValidProviderConfig(userConfig)) {
      provider = userConfig.provider;
      apiKey = userConfig.apiKey;
      model = userConfig.model;
    } else if (!apiKey) {
      /* No env key and no user config → graceful simulated fallback */
      return simulatedFallback(capability, prompt, projectContext, documentContext, summaryMode, roleContext);
    }
...
"""`
    ...
    const result = await runWingmanCompletion({
      provider,
      apiKey,
      model,
      systemInstruction,
    ...
    console.error('Wingman provider error:', error);
    return NextResponse.json(
      {
...
/* ── Graceful simulated fallback when no API key is configured ── */
...
- **Clause 4.2 / Table 2 (Building Envelope Insulation):**
  - Minimum total thermal resistance (Rt) for Climatic Zone 2 (Pretoria / Highveld):
    - Roof/Ceiling: Rt ≥ 3.70 m²·K/W (requires min 135mm mineral wool / EPS insulation).
    - External Solid Masonry Walls: Rt ≥ 0.35 m²·K/W (220mm cavity wall or thermal plaster required).
- **Clause 5.3 (Fenestration Area & Shading Factor):**
  - Storey 2 North-West fenestration ratio exceeds 15% net floor area threshold.
  - Mandatory solar shading overhang projection ratio P ≥ H × 0.35 or certified solar safety film (SHGC ≤ 0.50).
...
- **Architect (${roleContext || 'Lead Architect'}):** Update window schedule W-04 and W-05 to incorporate solar-control Low-E double glazing (U-value ≤ 2.4 W/m²·K, SHGC ≤ 0.48).
