...
export function InsuranceRegisterModule({ activeProject, currentRole, activeTabKey = 'policies', onTabChange }: Props) {
  ...
  return <section className="space-y-4" aria-label="Insurance Register">
    <PageHeader title="Insurance Register" origami={<OrigamiIcon name="insurance_register" size={26} />} metadata={<><p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#806000]">Coverage & renewals</p><p>{activeProject.code} · statutory and contractual cover</p></>} actions={<div className="flex max-w-full items-center gap-2"><Button type="button" variant="ink" size="sm" disabled={!canEdit} className="shrink-0">+ Add policy</Button><nav className="flex max-w-full gap-2 overflow-x-auto" aria-label="Insurance Register sections">{TABS.map(t => <Button key={t.key} type="button" variant={tab === t.key ? 'ink' : 'quiet'} size="sm" aria-pressed={tab === t.key} onClick={() => setTab(t.key || '')} className="shrink-0">{t.icon && <OrigamiIcon name={t.icon} size={14} />}{t.label}</Button>)}</nav></div>} />

    {tab === 'policies' && <div className="grid gap-4 lg:grid-cols-2">{policies.map(p => <article key={p.id} className="rounded-2xl border bg-white p-5 shadow-sm"><div className="flex items-start justify-between gap-3"><div><span className="font-mono text-xs font-bold text-[#806000]">{p.id}</span><h2 className="mt-1 text-sm font-bold">{p.policy}</h2><p className="mt-1 text-xs text-[#657287]">{p.insurer}</p></div><div className="text-right"><div className="text-sm font-bold">{p.cover}</div><span className={`mt-1 inline-block rounded-full px-2 py-1 text-[10px] font-bold ${p.status === 'Active' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-800'}`}>{p.status}</span></div></div><div className="mt-3 rounded-xl bg-[#f5faf9] p-3 text-xs"><strong>Expires:</strong> {p.expiry} <Button type="button" variant="quiet" size="sm" disabled={!canEdit} className="ml-2 text-[#135f5a]">Renew →</Button></div></article>)}</div>}
