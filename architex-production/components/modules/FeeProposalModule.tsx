...
export function FeeProposalModule({ activeProject, currentRole, activeTabKey = 'proposals', onTabChange }: Props) {
  ...
  return <section className="space-y-4" aria-label="Fee Proposal Builder">
    <header className="flex items-center justify-between rounded-2xl border bg-white p-5 shadow-sm">
      <div className="flex items-center gap-3"><div className="grid h-12 w-12 place-items-center rounded-2xl bg-amber-50 text-[#b77900]"><OrigamiIcon name="fee_proposal" size={26} /></div><div><p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#b77900]">Appoint-stage commercial terms</p><h1 className="text-xl font-bold">Fee Proposal Builder</h1><p className="text-xs text-[#657287]">{activeProject.code} · stage-based professional fees</p></div></div>
      <button disabled={!canEdit} className="rounded-xl bg-[#102033] px-4 py-2 text-xs font-bold text-white disabled:opacity-40">+ New Proposal</button>
    </header>
    ...
    {tab === 'proposals' && <div className="grid gap-4 lg:grid-cols-2">{items.map(p => <article key={p.id} className="rounded-2xl border bg-white p-5 shadow-sm"><div className="flex items-start justify-between gap-3"><div><span className="font-mono text-xs font-bold text-[#b77900]">{p.id}</span><h2 className="mt-1 text-sm font-bold">{p.title}</h2><p className="mt-2 text-xs text-[#657287]">Stage: {p.stage} · {p.date}</p></div><div className="text-right"><div className="text-lg font-bold text-[#102033]">{p.fee}</div><span className={`mt-1 inline-block rounded-full px-2 py-1 text-[10px] font-bold ${p.status === 'Accepted' ? 'bg-green-100 text-green-700' : p.status === 'Draft' ? 'bg-gray-100 text-gray-600' : 'bg-amber-100 text-amber-800'}`}>{p.status}</span></div></div><div className="mt-4 flex gap-2">{p.status === 'Draft' ? <button disabled={!canEdit} className="rounded-xl bg-[#b77900] px-3 py-2 text-xs font-bold text-white disabled:opacity-40">Send to client</button> : p.status === 'Pending client approval' ? <button disabled={!canEdit} className="rounded-xl border px-3 py-2 text-xs font-bold disabled:opacity-40">Withdraw</button> : <button disabled={!canEdit} className="rounded-xl border px-3 py-2 text-xs font-bold disabled:opacity-40">View</button>}</div></article>)}</div>}
