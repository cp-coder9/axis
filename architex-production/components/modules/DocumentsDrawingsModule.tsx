...
export function DocumentsDrawingsModule({ activeProject, currentRole, activeTabKey = 'register', onTabChange }: Props) {
  ...
  return <section className="space-y-4" aria-label="Documents and Drawings">
    <header className="flex flex-col gap-3 rounded-2xl border bg-white p-5 shadow-sm md:flex-row md:items-center md:justify-between">
      <div className="flex items-center gap-3"><div className="grid h-12 w-12 place-items-center rounded-2xl bg-[#DFF5F2] text-[#167E79]"><OrigamiIcon name="documents_drawings" size={26} /></div><div><p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#167E79]">Current-set control · {source === 'api' ? 'live' : 'offline seed'}</p><h1 className="text-xl font-bold">Documents & Drawings</h1><p className="text-xs text-[#657287]">{activeProject.code} · revisioned evidence shared across modules</p></div></div>
      <div className="flex gap-2">{['Register','Current Set','Transmittals','Markups'].map((l, i) => <button key={l} onClick={() => setTab(['register','current_set','transmittals','markups'][i])} className={`rounded-lg px-3 py-2 text-[11px] font-bold ${tab === ['register','current_set','transmittals','markups'][i] ? 'bg-[#19B7B0]/15 text-[#167E79]' : 'bg-[#f5faf9] text-[#657287]'}`}>{l}</button>)}</div>
    </header>
    ...
    {tab === 'register' && <>
      ...
      <div className="flex gap-1.5 overflow-x-auto">{[...filters].map(f => <button key={f} onClick={() => setFilter(f)} className={`rounded-full px-3 py-1.5 text-[11px] font-bold ${filter === f ? 'bg-[#102033] text-white' : 'bg-white border text-[#657287]'}`}>{f}</button>)}</div>
      <div className="overflow-hidden rounded-2xl border bg-white shadow-sm">
        <div className="overflow-x-auto"><table className="w-full min-w-[720px] text-left text-xs"><thead className="bg-[#f5faf9] text-[10px] uppercase text-[#657287]"><tr>{['Number','Title','Discipline','Revision','Purpose','Status','Updated',''].map(h => <th key={h} className="px-4 py-3">{h}</th>)}</tr></thead><tbody className="divide-y">{visible.map(doc => <tr key={doc.id} className={`hover:bg-[#f8fbfb] cursor-pointer ${selected === doc.id ? 'bg-[#DFF5F2]/40' : ''}`} onClick={() => setSelected(doc.id)}><td className="px-4 py-3 font-mono font-bold">{doc.number}</td><td className="px-4 py-3 font-semibold">{doc.title}</td><td className="px-4 py-3">{doc.discipline}</td><td className="px-4 py-3"><span className="rounded-full bg-[#DFF5F2] px-2 py-1 font-bold text-[#167E79]">{doc.rev}</span></td><td className="px-4 py-3">{doc.purpose}</td><td className="px-4 py-3">{doc.status}</td><td className="px-4 py-3 text-[#657287]">{doc.updated}</td><td className="px-4 py-3"><button data-testid={`doc-toggle-${doc.id}`} onClick={(e) => { e.stopPropagation(); void toggleStatus(doc); }} disabled={!canMutate || busyId === doc.id} className="rounded-lg border px-2 py-1 text-[10px] font-bold text-[#167E79] disabled:opacity-40">{busyId === doc.id ? '…' : doc.status === 'Approved' ? 'Unapprove' : 'Approve'}</button></td></tr>)}</tbody></table></div>
      </div>
