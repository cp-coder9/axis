...
export function ProfessionalDirectoryModule({ activeProject, currentRole, activeTabKey = 'directory', onTabChange }: Props) {
  ...
  return <section className="space-y-4" aria-label="Professional Directory">
    <PageHeader
      ...
      origami={<OrigamiIcon name="professional_directory" size={26} />}
      metadata={<><p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#167E79]">Council-verified professionals</p><p>SACAP · ECSA · SACQSP · SACPLAN · SACPCMP registrations</p></>}
      actions={<nav className="flex max-w-full overflow-x-auto" aria-label="Professional directory sections">{TABS.map(t => <Button key={t.key} type="button" variant={tab === t.key ? 'ink' : 'quiet'} size="sm" aria-pressed={tab === t.key} onClick={() => setTab(t.key || '')} className="shrink-0">{t.icon && <OrigamiIcon name={t.icon} size={14} />}{t.label}</Button>)}</nav>}
    />
    ...
    {tab === 'directory' && <><input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by name or discipline..." className="w-full rounded-2xl border border-[#102033]/10 bg-white p-3 text-xs shadow-sm focus:outline-none focus:border-[#19B7B0]" />
      <div className="grid gap-4 lg:grid-cols-2">{filtered.map(p => <article key={p.id} className="rounded-2xl border bg-white p-5 shadow-sm"><div className="flex items-start justify-between gap-3"><div className="flex items-center gap-3"><div className="grid h-11 w-11 place-items-center rounded-full bg-[#DFF5F2] text-xs font-bold text-[#135f5a]">{p.name.split(' ').map(n => n[0]).join('')}</div><div><div className="flex items-center gap-2"><h2 className="text-sm font-bold">{p.name}</h2>{p.verified && <span className="rounded-full bg-green-100 px-2 py-0.5 text-[9px] font-bold text-green-700">✓ Verified</span>}</div><p className="text-xs text-[#657287]">{p.discipline}</p></div></div><div className="text-right"><div className="font-mono text-[10px] text-[#657287]">{p.reg}</div><div className="text-[10px] font-bold text-[#135f5a]">{p.council}</div></div></div><div className="mt-3 flex items-center justify-between"><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${p.role ? 'bg-[#DFF5F2] text-[#135f5a]' : 'bg-gray-100 text-[#526074]'}`}>{p.role || 'Not appointed'}</span><button className="rounded-lg border px-2.5 py-1 text-[10px] font-bold text-[#135f5a]">View profile</button></div></article>)}</div></>}
