...
export function PaymentsEscrowModule({ activeProject, currentRole, activeTabKey = 'payments', onTabChange }: Props) {
  ...
  return <section className="space-y-4" aria-label="Payments and Escrow">
    <header className="flex items-center justify-between rounded-2xl border bg-white p-5 shadow-sm">
      <div className="flex items-center gap-3"><div className="grid h-12 w-12 place-items-center rounded-2xl bg-amber-50 text-[#b77900]"><OrigamiIcon name="payments_escrow" size={26} /></div><div><p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#b77900]">Workflow only · no fund holding</p><h1 className="text-xl font-bold">Payments & Escrow</h1><p className="text-xs text-[#657287]">{activeProject.code} · invoice, milestone and release-status tracking</p></div></div>
    </header>
    ...
    {tab === 'payments' && <div className="space-y-3">{items.map(p => <article key={p.id} className="flex flex-col gap-3 rounded-2xl border bg-white p-4 shadow-sm lg:flex-row lg:items-center"><div className="h-10 w-1 rounded-full bg-[#FFB020]" /><div className="flex-1"><div className="flex flex-wrap items-center gap-2"><span className="font-mono text-xs font-bold text-[#b77900]">{p.id}</span><h2 className="text-sm font-bold">{p.title}</h2></div><p className="mt-1 text-xs text-[#657287]">Payer: {p.payer} · Due {p.due}</p></div><div className="text-right"><div className="text-sm font-bold">{p.amount}</div><span className={`mt-1 inline-block rounded-full px-2 py-1 text-[10px] font-bold ${p.stage === 'Approved' || p.stage === 'Released' ? 'bg-green-100 text-green-700' : p.stage === 'Awaiting approval' ? 'bg-amber-100 text-amber-800' : 'bg-blue-100 text-blue-700'}`}>{p.stage}</span></div><button disabled={!canApprove || p.stage === 'Released'} onClick={() => advance(p.id)} className="rounded-xl border px-3 py-2 text-xs font-bold disabled:opacity-40">{p.stage === 'Prepared' ? 'Submit' : p.stage === 'Awaiting approval' ? 'Approve' : 'Released'}</button></article>)}</div>}
