...
export function MarketInsightsModule({ activeProject, currentRole, activeTabKey = 'market', onTabChange }: Props) {
  ...
  return <section className="space-y-4" aria-label="Market Insights">
    <header className="flex items-center justify-between rounded-2xl border bg-white p-5 shadow-sm">
      <div className="flex items-center gap-3"><div className="grid h-12 w-12 place-items-center rounded-2xl bg-[#DFF5F2] text-[#167E79]"><OrigamiIcon name="market_insights" size={26} /></div><div><p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#167E79]">Sector intelligence</p><h1 className="text-xl font-bold">Market Insights</h1><p className="text-xs text-[#657287]">{activeProject.code} · building cost, tender and material trends</p></div></div>
    </header>
    ...
    {tab === 'market' && <div className="grid gap-4 lg:grid-cols-3">{[['Residential construction cost index','Gauteng · Q2 2026','+6.2% YoY','up'],['Tender price index','Building sector','+4.8% YoY','up'],['Cement demand','National','-1.1% YoY','down'],['Steel (rebar) price','Y12 high-tensile','R 24,800/t · +3.4%','up'],['Labour cost index','Skilled trades','+5.5% YoY','up'],['Municipal approval times','Tshwane average','68 days · +6 days','up']].map(([title,meta,val,dir]) => <div key={title} className="rounded-2xl border bg-white p-5 shadow-sm"><h3 className="text-sm font-bold">{title}</h3><p className="mt-1 text-xs text-[#657287]">{meta}</p><p className={`mt-2 text-lg font-bold ${dir === 'up' ? 'text-[#d95747]' : 'text-green-700'}`}>{val}</p></div>)}</div>}
