...
export function SiteInstructionsModule({ activeProject, currentRole, activeTabKey = 'instructions', onTabChange }: Props) {
  ...
  return <section className="space-y-4" aria-label="Site Instructions">
    <PageHeader title="Site Instructions" origami={<OrigamiIcon name="site_instructions" size={26} />} metadata={<><p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#135f5a]">Formal architect/engineer directives</p><p>{activeProject.code} · contractor acknowledgement + cost implications</p></>} actions={<div className="flex max-w-full items-center gap-2"><Button type="button" variant="ink" size="sm" disabled={!canIssue} className="shrink-0">+ Issue Instruction</Button><nav className="flex max-w-full gap-2 overflow-x-auto" aria-label="Site Instructions sections">{TABS.map(t => <Button key={t.key} type="button" variant={tab === t.key ? 'ink' : 'quiet'} size="sm" aria-pressed={tab === t.key} onClick={() => setTab(t.key || '')} className="shrink-0">{t.icon && <OrigamiIcon name={t.icon} size={14} />}{t.label}</Button>)}</nav></div>} />
