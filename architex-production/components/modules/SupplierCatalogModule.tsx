...
export function SupplierCatalogModule({ activeProject, currentRole, activeTabKey = 'catalogue', onTabChange }: Props) {
  ...
  return <section className="space-y-4" aria-label="Supplier Catalogue">
    <header className="flex items-center justify-between rounded-2xl border bg-white p-5 shadow-sm">
      <div className="flex items-center gap-3"><div className="grid h-12 w-12 place-items-center rounded-2xl bg-[#DFF5F2] text-[#167E79]"><OrigamiIcon name="supplier_catalog" size={26} /></div><div><p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#167E79]">Verified supply chain</p><h1 className="text-xl font-bold">Supplier Catalogue</h1><p className="text-xs text-[#657287]">{activeProject.code} · compliance-checked suppliers with product data</p></div></div>
      <button className="rounded-xl bg-[#102033] px-4 py-2 text-xs font-bold text-white">+ Add supplier</button>
    </header>
    ...
    {tab === 'catalogue' && <div className="grid gap-4 lg:grid-cols-2">{items.map(s => <article key={s.id} className="rounded-2xl border bg-white p-5 shadow-sm"><div className="flex items-start justify-between gap-3"><div><span className="font-mono text-xs font-bold text-[#167E79]">{s.id}</span><h2 className="mt-1 text-sm font-bold">{s.name}</h2><p className="mt-1 text-xs text-[#657287]">Trade: {s.trade} · {s.orders} active orders</p></div><div className="text-right"><div className="text-lg font-bold">★ {s.rating}</div>{s.verified ? <span className="mt-1 inline-block rounded-full bg-green-100 px-2 py-1 text-[10px] font-bold text-green-700">✓ Verified</span> : <span className="mt-1 inline-block rounded-full bg-amber-100 px-2 py-1 text-[10px] font-bold text-amber-800">Pending verification</span>}</div></div><div className="mt-4 flex gap-2"><button disabled={!canVerify} onClick={() => toggleVerify(s.id)} className="rounded-xl border px-3 py-2 text-xs font-bold disabled:opacity-40">{s.verified ? 'Revoke' : 'Verify'}</button><button className="rounded-xl bg-[#19B7B0] px-3 py-2 text-xs font-bold text-white">View products</button></div></article>)}</div>}
