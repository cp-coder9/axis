...
export function RfqMarketplaceModule({ activeProject, currentRole, activeTabKey = 'rfqs', onTabChange }: Props) {
  ...
  return <section className="space-y-4" aria-label="RFQ Marketplace">
    <header className="flex items-center justify-between rounded-2xl border bg-white p-5 shadow-sm">
      <div className="flex items-center gap-3"><div className="grid h-12 w-12 place-items-center rounded-2xl bg-[#DFF5F2] text-[#167E79]"><OrigamiIcon name="rfq_marketplace" size={26} /></div><div><p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#167E79]">Package-based procurement</p><h1 className="text-xl font-bold">RFQ Marketplace</h1><p className="text-xs text-[#657287]">{activeProject.code} · controlled requests, transparent comparisons</p></div></div>
      <button disabled={!canManage} className="rounded-xl bg-[#102033] px-4 py-2 text-xs font-bold text-white disabled:opacity-40">+ New RFQ</button>
    </header>
    ...
    {tab === 'rfqs' && <div className="space-y-3">{items.map(r => <article key={r.id} className="flex flex-col gap-3 rounded-2xl border bg-white p-4 shadow-sm lg:flex-row lg:items-center"><div className="h-10 w-1 rounded-full bg-[#19B7B0]" /><div className="flex-1"><div className="flex flex-wrap items-center gap-2"><span className="font-mono text-xs font-bold text-[#167E79]">{r.id}</span><h2 className="text-sm font-bold">{r.title}</h2><span className="rounded-full bg-[#DFF5F2] px-2 py-1 text-[10px] font-bold text-[#167E79]">{r.trade}</span></div><p className="mt-1 text-xs text-[#657287]">Closes {r.deadline} · {r.quotes} quotes received</p></div><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${r.status === 'Awarded' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-800'}`}>{r.status}</span><button disabled={!canManage} className="rounded-xl border px-3 py-2 text-xs font-bold disabled:opacity-40">{r.status === 'Open' ? 'Compare quotes' : 'View award'}</button></article>)}</div>}
