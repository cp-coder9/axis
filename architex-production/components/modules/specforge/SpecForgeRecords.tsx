...
import type { CreateSpecForgeItemInput, CreateSpecForgeSectionInput } from '@/lib/specforge/api';
...
interface Props {
  ...
  onUpdateItem: (itemId: string, patch: Partial<CreateSpecForgeItemInput>, expectedVersion: number) => Promise<unknown>;
...
export function SpecForgeRecords({ tab, workspace, role, canEdit, canIssue, canReviewBudget, onCreateSection, onDuplicate, onUpdateItem, onUpdateBoq, onTransitionProcurement, onDecide, onConfirmResponsibility, onValidateIssue, onIssue, onListJobs, onDrawingScan }: Props) {
  ...
  const detail = selectedItem && <ItemDetail item={selectedItem} canEdit={canEdit} onUpdate={onUpdateItem} onDuplicate={onDuplicate} onClose={() => setSelectedItemId(null)} />;

  if (tab === 'pictorial') return <section data-tool-tab={tab} className="specforge-record-view">{filters}<div className="specforge-pictorial">{filteredItems.map(item => { const delta = item.estimatedCost - item.budgetAllowance; return <Surface as="article" level="raised" key={item.id}><button type="button" className="specforge-product-card" aria-label={`Open ${item.title} details`} onClick={() => setSelectedItemId(item.id)}><div className="specforge-product-image">{item.imageUrl ? <div className="specforge-product-photo" role="img" aria-label={item.title} style={{ backgroundImage: cssImage(item.imageUrl) }} /> : <div><span>{item.finish ?? 'Product image pending'}</span><small>{item.dimensions ?? item.packageName}</small></div>}</div><div className="specforge-product-copy"><div><code>{item.code}</code>{status(item.status)}</div><h2>{item.title}</h2><p>{item.supplier ?? 'Supplier not assigned'} · {item.model ?? 'performance basis'}</p><small>{item.room} · {item.sourceRevision}</small><div className="specforge-risk-flags">{delta > 0 && <span>Over allowance</span>}{item.leadTimeDays >= 56 && <span>{item.leadTimeDays}d lead</span>}{item.supersededBy && <span>Superseded</span>}</div><footer><strong className={delta > 0 ? 'is-risk' : ''}>{delta > 0 ? '+' : delta < 0 ? '−' : ''}{money(Math.abs(delta))}</strong><span>{item.leadTimeDays} days</span></footer></div></button></Surface>; })}{filteredItems.length === 0 && <Empty title="No matching specifications" detail="Change the search or project filters." />}</div>{detail}</section>;
...
function ItemDetail({ item, canEdit, onUpdate, onDuplicate, onClose }: { item: SpecForgeItem; canEdit: boolean; onUpdate: (itemId: string, patch: Partial<CreateSpecForgeItemInput>, expectedVersion: number) => Promise<unknown>; onDuplicate: (itemId: string) => Promise<unknown>; onClose: () => void }) {
  const [editing, setEditing] = useState(false);
  if (editing) return <ItemEditor item={item} onClose={() => setEditing(false)} onSave={async patch => { await onUpdate(item.id, patch, item.lockVersion); setEditing(false); }} />;
  return <div className="specforge-detail-backdrop" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}><Surface as="section" level="raised" className="specforge-item-detail" role="dialog" aria-modal="true" aria-labelledby="specforge-item-detail-title"><header><div><code>{item.code}</code><h2 id="specforge-item-detail-title">{item.title}</h2></div><Button type="button" variant="quiet" size="sm" aria-label="Close item details" onClick={onClose}>Close</Button></header>{item.imageUrl ? <div className="specforge-detail-image" role="img" aria-label={item.title} style={{ backgroundImage: cssImage(item.imageUrl) }} /> : <div className="specforge-detail-image specforge-detail-image--empty"><span>Product image pending</span><small>Integration required</small></div>}<p className="specforge-detail-description">{item.description}</p><dl><div><dt>Room</dt><dd>{item.room}</dd></div><div><dt>Package</dt><dd>{item.packageName}</dd></div><div><dt>Supplier</dt><dd>{item.supplier ?? 'Unassigned'}</dd></div><div><dt>Model</dt><dd>{item.model ?? 'Open'}</dd></div><div><dt>Finish</dt><dd>{item.finish ?? 'Open'}</dd></div><div><dt>Dimensions</dt><dd>{item.dimensions ?? 'Open'}</dd></div><div><dt>Budget</dt><dd>{money(item.budgetAllowance)}</dd></div><div><dt>Estimate</dt><dd>{money(item.estimatedCost)}</dd></div><div><dt>Lead time</dt><dd>{item.leadTimeDays} days</dd></div><div><dt>Status</dt><dd>{status(item.status)}</dd></div><div><dt>Source revision</dt><dd>{item.sourceRevision}</dd></div><div><dt>Owner</dt><dd>{item.ownerRole.replaceAll('_', ' ')}</dd></div></dl>{canEdit && <footer><Button type="button" onClick={() => setEditing(true)}>Edit specification</Button><Button type="button" variant="secondary" onClick={() => void onDuplicate(item.id)}>Duplicate as draft</Button></footer>}</Surface></div>;
}

function ItemEditor({ item, onSave, onClose }: { item: SpecForgeItem; onSave: (patch: Partial<CreateSpecForgeItemInput>) => Promise<void>; onClose: () => void }) {
  const [values, setValues] = useState({ title: item.title, room: item.room, packageName: item.packageName, description: item.description, supplier: item.supplier ?? '', model: item.model ?? '', finish: item.finish ?? '', dimensions: item.dimensions ?? '', budgetAllowance: String(item.budgetAllowance), estimatedCost: String(item.estimatedCost), leadTimeDays: String(item.leadTimeDays), clientDecision: item.clientDecision });
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const field = (key: keyof typeof values, value: string | boolean) => setValues(current => ({ ...current, [key]: value }));
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault(); setBusy(true); setError(null);
    try { await onSave({ title: values.title.trim(), room: values.room.trim(), packageName: values.packageName.trim(), description: values.description.trim(), supplier: values.supplier.trim() || null, model: values.model.trim() || null, finish: values.finish.trim() || null, dimensions: values.dimensions.trim() || null, budgetAllowance: Number(values.budgetAllowance), estimatedCost: Number(values.estimatedCost), leadTimeDays: Number(values.leadTimeDays), clientDecision: values.clientDecision }); }
    catch (caught) { setError(caught instanceof Error ? caught.message : 'Specification could not be saved.'); }
    finally { setBusy(false); }
  };
  return <div className="specforge-detail-backdrop" role="presentation"><Surface as="section" level="raised" className="specforge-item-editor" role="dialog" aria-modal="true" aria-labelledby="specforge-item-editor-title"><header><div><code>{item.code} · version {item.lockVersion}</code><h2 id="specforge-item-editor-title">Edit specification</h2></div><Button type="button" variant="quiet" size="sm" aria-label="Close specification editor" onClick={onClose}>Close</Button></header><form onSubmit={event => void submit(event)}><label>Specification title<input aria-label="Specification title" value={values.title} onChange={event => field('title', event.target.value)} maxLength={220} required /></label><label>Room<input aria-label="Room" value={values.room} onChange={event => field('room', event.target.value)} maxLength={180} /></label><label>Package<input aria-label="Package" value={values.packageName} onChange={event => field('packageName', event.target.value)} maxLength={180} /></label><label>Supplier<input aria-label="Supplier" value={values.supplier} onChange={event => field('supplier', event.target.value)} maxLength={180} /></label><label>Model<input aria-label="Model" value={values.model} onChange={event => field('model', event.target.value)} maxLength={180} /></label><label>Finish<input aria-label="Finish" value={values.finish} onChange={event => field('finish', event.target.value)} maxLength={180} /></label><label>Dimensions<input aria-label="Dimensions" value={values.dimensions} onChange={event => field('dimensions', event.target.value)} maxLength={180} /></label><label>Budget allowance<input aria-label="Budget allowance" type="number" min="0" step="0.01" value={values.budgetAllowance} onChange={event => field('budgetAllowance', event.target.value)} required /></label><label>Estimated cost<input aria-label="Estimated cost" type="number" min="0" step="0.01" value={values.estimatedCost} onChange={event => field('estimatedCost', event.target.value)} required /></label><label>Lead time days<input aria-label="Lead time days" type="number" min="0" max="3650" step="1" value={values.leadTimeDays} onChange={event => field('leadTimeDays', event.target.value)} required /></label><label className="specforge-item-editor__description">Description<textarea aria-label="Description" value={values.description} onChange={event => field('description', event.target.value)} maxLength={65535} required /></label><label className="specforge-item-editor__check"><input aria-label="Client decision required" type="checkbox" checked={values.clientDecision} onChange={event => field('clientDecision', event.target.checked)} /> Client decision required</label>{error && <p role="alert">{error}</p>}<footer><Button type="submit" busy={busy}>Save specification</Button><Button type="button" variant="secondary" onClick={onClose}>Cancel</Button></footer></form></Surface></div>;
}
