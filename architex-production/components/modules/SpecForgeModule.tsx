  ...
  return (
    <section className="specforge-v8" aria-label="SpecForge specification workspace" data-specforge-state={state.status}>
      ...
      {workspace && (state.status === 'ready' || state.status === 'conflict') && <>
        {smartAddOpen && canEdit && <SpecForgeSmartAdd section={workspace.sections[0] ?? null} revision={workspace.revision} onConfirm={state.actions.createItem} onRequestDrawingScan={state.actions.requestDrawingScan} onClose={() => setSmartAddOpen(false)} />}
        {tab === 'overview' ? <SpecForgeOverview workspace={workspace} onOpenTab={setTab} /> : <SpecForgeRecords tab={tab} workspace={workspace} role={effectiveAuthorizationRole} canEdit={canEdit} canIssue={canIssue} onCreateSection={state.actions.createSection} onDuplicate={state.actions.duplicateItem} onDecide={(approvalId, decision) => state.actions.decideApproval(approvalId, decision, null)} onValidateIssue={state.actions.validateIssue} onIssue={state.actions.issue} onListJobs={state.actions.listJobs} onDrawingScan={state.actions.requestDrawingScan} />}
      </>}
    </section>
  );
}
